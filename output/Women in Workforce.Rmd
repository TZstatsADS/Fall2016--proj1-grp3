...
Data$Income_m<-ifelse(Data$Income>=30000,"2.above","1.below")

#get the data frame needed: education, race effect on income(below/above)
names(Data)
Data_women<-subset(Data,Gender=='2')
attach(Data_women)
Data_women<-data.frame(Education,Race,Income_m,Weight)
detach(Data_women)
...
xlab = "Educational attainment", names = c("~grade8", "grade9~nodegree","associate/bachelor","professional/master","doctor"), 
ylab = "proportion", legend = c("Below median income", "Above median income"), 
args.legend = list(title = "Educational attainment", x = "topright", cex = 1), ylim = c(0, 1.5))
##Below percentage decreases at higher educational attainment
##Without dgree more than half are below median income

fit_edu<-svyglm(factor(Income_m) ~ factor(Education), design = Data_women_w,family=binomial)
summary(fit_edu)
##every factor is significant - interpretation on logit scale
##Education2, versus Education1(~grade8:baseline) multiplies the odds of being above $30000 group by exp(1.03)
##???at higher educational attainment, the chance of being ¡°above $30000¡± group increases
##gap between 2-3 is the largest, college degree is important to be in above $30000 group!
##I would categorize "women without college degree" as vulnerable women
```
